...
void GSGame::onFrameRender(CL_GraphicContext* gc){
	...
	m_background->draw(*gc, (float)pos.x, (float)pos.y);
	...
	for(int i = 0; i < 3; ++i){
		...
		if(m_bullets.state[i] != BULLET_STATE_INACTIVE){
			m_testbullet->draw(*gc, (float)pos.x, (float)pos.y);
		}
	}
	...
	if(m_tanks.state[m_playertank] != TANK_STATE_INACTIVE){
		...
		m_testtank->draw(*gc, (float)pos.x, (float)pos.y);
	}
	...
	if(m_tanks.state[m_player2tank] != TANK_STATE_INACTIVE){
		...
		m_testtank2->draw(*gc, (float)pos.x, (float)pos.y);
	}
