...
// 	cout << "bullet_vector tank_vector direction_state distance_state | tank.x tank.y tank.rot | bullet.x bullet.y bullet.rot" << endl;
	...
	u32 score = 0;
	// set the gene data
	g_db->get_gene_data(g_aik, score, this->get_ai(), m_scenario_results);
...
void GSGeneView::prepare_game_scenario(u32 dist, u32 bullet_loc, u32 bullet_vec){
	...
	m_index = (dist * NUM_LOCATION_STATES * NUM_BULLET_VECTORS)
				+ (bullet_loc * NUM_BULLET_VECTORS)
				+ bullet_vec;
...
// 				   sizeof(m_player_input), cudaMemcpyHostToDevice);
		...
		if(m_tanks.state[m_playertank] == TANK_STATE_INACTIVE){
			...
			<< " " << m_scenario_results[m_index] << " dead" << endl;
			++m_test_vect;
			if(m_test_vect >= 1){
				m_test_vect = 0;
				++m_test_sect;
			...
			cout << m_test_dist << " " << m_test_sect << " " << m_test_vect
			<< " " << m_scenario_results[m_index] << " alive" << endl;
			++m_runningscore;
			++m_test_vect;
			if(m_test_vect >= 1){
				m_test_vect = 0;
