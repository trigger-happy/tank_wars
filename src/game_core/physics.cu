...
PhysObject::PhysObject(PhysRunner* p) : m_runner(p){
...
vec2 PhysObject::get_acceleration(){
	vec2 temp;
...
f32 PhysObject::get_rotation(){
	f32 rot = m_runner->m_hostbodies.rotation[m_objid];
	return rot;
}

f32 PhysObject::get_max_velocity(){
	f32 mv = m_runner->m_hostbodies.rotation[m_objid];
	return mv;
}

bool PhysObject::is_collidable(){
	bool f = m_runner->m_hostbodies.can_collide[m_objid];
	return f;
}

void PhysObject::set_cur_pos(const vec2& pos){
